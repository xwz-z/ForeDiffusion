...
from foresight_conditioned.model.diffusion.conv1d_components import (
    Downsample1d, Upsample1d, Conv1dBlock)
from foresight_conditioned.model.diffusion.positional_embedding import SinusoidalPosEmb
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        self.mid_modules = nn.ModuleList([
            ConditionalResidualBlock1D(
                mid_dim, mid_dim, cond_dim=int(cond_dim*4/3),
                kernel_size=kernel_size, n_groups=n_groups,
                condition_type=condition_type
            ),
            ConditionalResidualBlock1D(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            down_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(
                    dim_in, dim_out, cond_dim=int(cond_dim*4/3), 
                    kernel_size=kernel_size, n_groups=n_groups,
                    condition_type=condition_type),
                ConditionalResidualBlock1D(
                    dim_out, dim_out, cond_dim=int(cond_dim*4/3), 
                    kernel_size=kernel_size, n_groups=n_groups,
                    condition_type=condition_type),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            up_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(
                    dim_out*2, dim_in, cond_dim=int(cond_dim*4/3),
                    kernel_size=kernel_size, n_groups=n_groups,
                    condition_type=condition_type),
                ConditionalResidualBlock1D(
                    dim_in, dim_in, cond_dim=int(cond_dim*4/3),
                    kernel_size=kernel_size, n_groups=n_groups,
                    condition_type=condition_type),
...
class GlobalConditionPredictor(nn.Module):
    def __init__(self):
        super(GlobalConditionPredictor, self).__init__()
        
        self.model = nn.Sequential(
            nn.Linear(256, 512),   # 先升维
            nn.ReLU(inplace=True),
            nn.Linear(512, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 128)    # 最后降到128
        )
...
class AttentionPooling(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.attention = nn.Linear(feature_dim, 1)  # attention weight

    def forward(self, x):
        weights = torch.softmax(self.attention(x), dim=1)  # (batch_size, 3, 1)
        x = (x * weights).sum(dim=1)  # 加权求和 (batch_size, feature_dim)
        return x

class NextPosePredictor(nn.Module):
    def __init__(self, input_dim=1024, num_layers=2, num_heads=4, hidden_dim=128):
        super().__init__()
